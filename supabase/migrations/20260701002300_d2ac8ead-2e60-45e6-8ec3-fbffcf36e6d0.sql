...
CREATE OR REPLACE FUNCTION public.bulk_upsert_classes_with_roster(_payload jsonb)
 RETURNS TABLE(class_id uuid, class_name text, inserted_count integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      ELSE
        -- upsert by (class_id, student_no) via manual update-then-insert
        UPDATE public.class_roster
           SET student_name = v_name
         WHERE class_id = v_class_id AND student_no = v_no;
        IF NOT FOUND THEN
          INSERT INTO public.class_roster (class_id, student_no, student_name)
